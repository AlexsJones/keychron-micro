...
🟡 yellow while a script runs, 🟢 green when it exits cleanly, 🔴 red when it
fails, 🟣 magenta when something wants you — driven over VIA raw HID, in RAM, so
your saved VIA config is never touched. Any process can colour it by writing one
word to a fifo, which is how a Claude Code session running on the pad reports
itself: the board is yellow while it thinks and green when it hands back.
...
|---|---|---|
| Cost | **$230**, while supplies last | **free** — on a [Q0 Max](https://www.keychron.com/products/keychron-q0-max-qmk-custom-number-pad) (~$110), or £0 if you have one |
| Source | closed | open, ~900 lines you can read in a sitting |
| Runs on | **Mac / Windows only** | Linux / Wayland |
| Tied to | the ChatGPT desktop app | any agent — `command = "codex"` and it is codex |
...
the hardware, buy the hardware — it looks lovely. If you run Linux, where Codex
Micro does not go at all, this is the whole thing for the price of a numpad.
...
The name appears on the page — `KEY_KP8`, say. It reports unbound keys too,
which is exactly when you need it. (`make learn` does the same in a terminal,
but has to stop the daemon first: both want an exclusive grab.)
...
**3. Save.** It applies at once — no restart, no dropped keypress. A config that
would not load is rejected and never written, so a typo costs you a red message
rather than a dead pad on the next reboot.

### The three fields

- **`key`** — the evdev name from step 1.
- **`label`** — documentation only, and worth writing. The legends and the
...
- **`run`** — relative to this repo, so a clone reproduces your setup as-is.
...
- **`M1`–`M4`** (the left column) fire *macros*. An empty macro sends nothing at
  all, so those keys are dead until you give them a keycode — any unused one
  will do, since nothing escapes the grab.
- **`MO(1)`** switches layer inside the firmware and sends no keycode ever. It
...
⚠️ **The daemon runs with a bare `PATH`** — a systemd user unit gets no
`~/.local/bin`, because nothing sources your shell profile. Anything in
`scripts/lib/term.sh` is fine (it runs a login shell), but a script calling a
tool from `~/.local/bin` directly will not find it. Use `bash -lc`, as the
scripts here do.

Two helpers worth using:

- **`lib/term.sh [--no-hold] <cmd>`** — opens a window with whatever terminal
  the machine has: Ptyxis on Fedora Workstation, Konsole on the KDE spin,
  `xterm` as the floor. `$TERMINAL` overrides.
- **`lib/status.sh <state>`** — colours the board (below). No-ops when the daemon
  is not running, so a macro key never fails for want of lights.
...
`presets/` is the tracked half — snapshots you chose to share.
...
- **`default`** — the demo key only. Nothing runs an agent until you say so.
- **`alexsjones`** — a full pad: agent sessions, tmux fleet, overview, terminals.
...
the session the pad launched and nothing else. Your other sessions are untouched
and `~/.claude/settings.json` is never written to.
...
One edit, and every script follows — they read the harness from the daemon
...
- **Focus a named window** — no. `Shell.Eval` is disabled since GNOME 41,
  `GetWindows` and `FocusSearch` answer *"is not allowed"*, and `wmctrl`/
...
GNOME binds *key combinations*, not *devices* — it cannot tell that `Q` came
from the pad rather than your keyboard, so every binding fires from both. It is
also dconf-only, GNOME-only, and per-machine.

## Why not keyd

[keyd](https://github.com/rvaiya/keyd) is the obvious tool, but it is not
packaged for Fedora, and its `command()` runs as the user running keyd — root —
which is both a security smell and awkward for launching GUI apps that need your
Wayland session. This daemon runs as you.
...
- **The udev rule** grants access to VID:PID `3434:0800` only — this pad, never
  your real keyboard. Fedora deliberately withholds `uaccess` from keyboards:
  read access to a keyboard is read access to everything you type. Joining the
  `input` group would work too, and would grant access to *every* input device.
...
- **Lighting** is set over VIA raw HID (usage page `0xFF60`) and lives in RAM.
  It never sends `id_custom_save`, so nothing survives a replug and your saved
  VIA config cannot be clobbered.
...
`tmux` only for `agents-tmux.sh`. Nothing else — the daemon is `evdev`, `libc`,
`serde` and a hand-rolled HTTP server.
